...
const Navbar = () => {
  ...
  const navLinks = [
    ...
    { name: 'Health Checkup', href: '/health-checkup', active: false },
  ...
  return (
    <header className={`w-full font-sans fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
      !isTop ? 'bg-white shadow-lg' : 'bg-[#e0fffd]'
    }`}>
      
      {/* TOP INFO BAR - Only visible at top */}
      <div className={`transition-all duration-500 overflow-hidden ${
        isTop ? 'max-h-32 opacity-100' : 'max-h-0 opacity-0'
      }`}>
        <div className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-16 py-2 flex flex-col sm:flex-row items-center justify-between gap-2 sm:gap-0 border-b border-[#059669]/10">
          ...
          <div className="hidden md:flex items-center justify-center sm:justify-start gap-3 md:gap-4 lg:gap-8 text-[11px] md:text-[12px] lg:text-[13px] font-medium text-slate-600 w-full sm:w-auto">
            ...
            <div className="flex items-center gap-1.5 group cursor-pointer">
              ...
              <a href="mailto:hello@yoursite.com" className="hover:text-[#059669] transition-colors text-[10px] sm:text-[11px] md:text-[12px]">
                evergreen@gmail.com
              </a>
            </div>
          </div>

          {/* Right: Social/WA - Visible on all screens */}
      ...
      <div className={`transition-all duration-500 ${
        !isTop ? 'py-1 sm:py-2' : 'py-2 sm:py-4'
      }`}>
        <div className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-16">
          <nav className={`rounded-[20px] px-3 sm:px-4 md:px-6 lg:px-8 py-2 sm:py-3 flex items-center justify-between transition-all duration-500 ${
            !isTop 
              ? 'bg-white' 
            ...
            <Link to="/" className="flex items-center flex-shrink-0">
              <img 
                src={logoImg} 
                alt="Evergreen Logo" 
                className={`object-contain hover:opacity-90 transition-opacity cursor-pointer transition-all duration-500 ${
                  !isTop ? 'h-10 sm:h-12' : 'h-10 sm:h-12'
                }`}
              />
            </Link>
            ...
            <ul className="hidden lg:flex items-center gap-6 xl:gap-10">
              {activeNavLinks.map((link) => (
                <li key={link.name}>
                  <Link
                    to={link.href}
                    className={`relative text-[13px] xl:text-[15px] font-semibold tracking-wide transition-all duration-300 group ${
                      link.active ? 'text-[#009fab]' : 'text-slate-800 hover:text-[#14B8A6]'
                    }`}
                  >
                    {link.name}
                    <span className={`absolute -bottom-1 left-0 h-[2.5px] bg-[#14B8A6] rounded-full transition-all duration-300 ${
            ...
            <div className="flex items-center gap-2 sm:gap-3 lg:gap-6">
              ...
              <div className="hidden md:flex items-center gap-1 sm:gap-2 lg:gap-3">
                <div className={`w-7 h-7 sm:w-8 sm:h-8 lg:w-11 lg:h-11 rounded-full flex items-center justify-center border shadow-inner transition-all duration-500 ${
                  !isTop 
                    ? 'bg-[#E0F7F9] border-[#14B8A6]/20' 
                    : 'bg-[#E0F7F9] border-[#14B8A6]/20'
                }`}>
                  <BiPhoneCall className="text-[#009fab] text-xs sm:text-sm lg:text-xl" />
                </div>
                <div className="flex flex-col">
                  ...
                  <span className={`font-extrabold text-slate-900 leading-tight tracking-tight transition-all duration-500 whitespace-nowrap ${
                    !isTop ? 'text-[10px] sm:text-[11px] lg:text-[14px]' : 'text-[11px] sm:text-[12px] lg:text-[17px]'
                  }`}>
                    1 800-657-876
                  </span>
                </div>
              ...
              {/* Appointment Button */}
              <Link to="/appointment">
                <button className={`rounded-[12px] font-bold shadow-[0_10px_20px_rgba(20,184,166,0.2)] hover:bg-[#0f172a] hover:shadow-none hover:-translate-y-0.5 active:translate-y-0 transition-all duration-300 whitespace-nowrap ${
                  !isTop 
      ...
      {/* Mobile Menu Panel */}
      <div className={`mobile-menu fixed top-0 right-0 h-full w-72 sm:w-80 bg-white shadow-2xl z-50 transition-transform duration-300 transform ${
          ...
          <ul className="space-y-4">
            {activeNavLinks.map((link) => (
              <li key={link.name}>
                <Link
                  to={link.href}
                  className={`block text-center py-3 text-[16px] font-semibold transition-colors rounded-lg hover:bg-slate-50 ${
                    link.active ? 'text-[#009fab] bg-slate-50' : 'text-slate-700 hover:text-[#14B8A6]'
                  }`}
                  onClick={() => setIsMobileMenuOpen(false)}
                >
                  {link.name}
